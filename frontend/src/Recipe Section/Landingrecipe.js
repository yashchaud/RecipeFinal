...
const Test = () => {
  ...
  const colors = ["rgb(49,47,61)", "rgb(56,50,85)", "rgb(65,60,87)", "rgb(57,54,73)", "rgb(37,35,47)", "rgb(18,15,31)"];
  ...
  const colors2 = ["rgb(191,182,62)", "rgba(191,182,62,1)", "rgba(223,216,42,1)", "rgba(193,217,52,1)", "rgba(143,198,48,1)"];
  useEffect(() => {
    console.log(instructionsref.current.children[0].children[0]);
    const t1 = gsap.timeline({ defaults: { ease: "Expo.easeInOut" } });
    t1.to(seconddiv.current, { "--color1": colors, backgroundPosition: "200% 100%", duration: 2, repeat: -1, yoyo: true });
  ...
  const handleLike = async () => {
    await axios
      .post(`/users/like`, { recipeId: data[0]._id })
      .then((res) => {
        console.log(res);
      })
      .catch((err) => {
      ...
      <Titlecover>
        ...
        <Titletext>
          <Actualtextdiv ref={instructionsdiv}>
            ...
            <h1 ref={titleref}>{data[index].name}</h1>
