...
const Newapproach = () => {
  ...
  const Animation = (index) => {
    ...
    const t1 = Flip.from(state, {
      ...
      onComplete: () => {
        ...
        gsap.fromTo(
          [divre.current.children[0].children[0], detailse.words],
          { duration: 1, opacity: 0, y: 200, delay: 0.5 },
          {
            duration: 1,
            opacity: 1,
            y: -0,
  ...
  useEffect(() => {
    ...
    img.current.to(divre.current.children[1].children, { xPercent: -100 * (4 - 1), ease: "none" });
  }, []);
  ...
  return (
    <div>
      ...
      <Cover ref={divre}>
        <Details>
          {data && (
            <Link className="linkButton" to={`/test/${index}`}>
              {data[index].name}
            </Link>
          )}
        </Details>
        ...
        <Container ref={containerRef}>
          {Imagearr.map((item, index) => {
            return <img key={index} src={item} alt="" onClick={() => Animation(index)} />;
          })}
        </Container>
      </Cover>
    </div>
